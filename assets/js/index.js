...
function prepareDeleteCommentForm(e){
    $("#delete_comment_form").find("#comment_id_input").val(
        $(this).closest(".comment_item").attr("data-comment-item")
    );
...
function prepareDeleteMessageForm(e){
    $("#delete_message_form").find("#message_id_input").val(
        $(this).closest(".message_item").attr("data-message-item")
    );
...
function prepareCommentForm(e){
    $("#comment_form").find("#message_id_input").val(
        $(this).closest(".message_item").attr("data-message-item")
    );

    $("#comment_form").find("#comment_input").val(
        $(this).siblings(".textarea_comment").val()
    );
...
function submitDeleteMessageForm(e){
    ...
    if(parseInt(is_processing) === 0){
        $(delete_message_form).attr("data-is-processing", 1)

        $.post(delete_message_form.attr("action"), delete_message_form.serialize(), function(data){
            if(data.status){
...
function submitDeleteCommentForm(e){
    ...
    if(parseInt(is_processing) === 0){
        $(delete_comment_form).attr("data-is-processing", 1)

        $.post(delete_comment_form.attr("action"), delete_comment_form.serialize(), function(data){
            if(data.status){
                window.location.href = "/home"
            }
            else{
